...
export const initData = async () => {
  ...
  try {
    ...
    const keys: { [key: string]: { publicJwk: JWK_OKP_Public; privateJwk: JWK_OKP_Private } }[] = await Promise.all(
      issuers.map(async ({ id, kid }: DataIssuer) => {
        const { publicKey, privateKey } = await generateKeyPair("Ed25519", {
          extractable: true,
        })
        const publicJwk = (await exportJWK(publicKey)) as JWK_OKP_Public
        const privateJwk = (await exportJWK(privateKey)) as JWK_OKP_Private
        publicJwk.kid = kid
        privateJwk.kid = kid
